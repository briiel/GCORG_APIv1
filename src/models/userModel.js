...
const getAllUsers = async () => {
  const [students] = await db.query(
    'SELECT id, email, first_name, last_name, middle_initial, suffix, department, program, COALESCE(year_level, 4) AS year_level, "student" as userType, NULL as org_name FROM students',
  );
  const [organizations] = await db.query(
    'SELECT id, email, org_name as name, "organization" as userType, department FROM student_organizations',
  );
...
const getAllUsersPaginated = async (page = 1, per_page = 20) => {
  ...
  // Use UNION ALL to combine both tables into one selectable dataset, filtering out soft-deleted orgs
...
const getUserById = async (id) => {
  const [student] = await db.query(
    'SELECT id, email, first_name, last_name, middle_initial, suffix, department, program, COALESCE(year_level, 4) AS year_level, "student" as userType, NULL as org_name FROM students WHERE id = ?',
    [id],
  );
  if (student.length > 0) return decryptUserFields(student[0]);

  const [organization] = await db.query(
    'SELECT id, email, org_name as name, "organization" as userType, department FROM student_organizations WHERE id = ?',
    [id],
  );
...
const getOrganizationMembersPaginated = async (
  orgId,
  page = 1,
  per_page = 20,
) => {
