...
const resolveScannedBy = (row) => {
  ...
  if (
    row._officer_first_name !== undefined ||
    row._officer_last_name !== undefined
  ) {
...
const getAttendanceRecordsByEvent = async (eventId) => {
  const query = `
        ...
        LEFT JOIN organizationmembers om ON ar.scanned_by_student_id = om.student_id AND om.is_active = TRUE
        WHERE ar.event_id = ?
    `;
  try {
...
const createEvent = async (eventData) => {
  ...
  try {
    const [result] = await db.query(query, [
      ...
      event_latitude !== undefined &&
      event_latitude !== null &&
      event_latitude !== ""
        ? Number(event_latitude)
        : null,
      event_longitude !== undefined &&
      event_longitude !== null &&
      event_longitude !== ""
        ? Number(event_longitude)
        : null,
      startDateStr,
      start_time,
      endDateStr,
      ...
      created_by_org_id &&
      created_by_org_id !== "undefined" &&
      created_by_org_id !== ""
        ? created_by_org_id
        : null,
      created_by_osws_id &&
      created_by_osws_id !== "undefined" &&
      created_by_osws_id !== ""
        ? created_by_osws_id
        : null,
      // created_by_student_id may be provided by controller to capture the officer/member who created the event
      eventData.created_by_student_id &&
      eventData.created_by_student_id !== "undefined" &&
      eventData.created_by_student_id !== ""
        ? eventData.created_by_student_id
        : null,
    ...
    const [rows] = await db.query(
      "SELECT MAX(event_id) AS last_id FROM created_events",
    );
    const lastId =
      rows && rows[0] && rows[0].last_id != null
        ? Number(rows[0].last_id)
        : null;
    if (!lastId)
      console.error(
        "[createEvent] insertId was 0/null and MAX fallback also failed.",
      );
    return lastId;
  } catch (error) {
  ...
  try {
    if (page && per_page) {
      ...
      const [[countRow]] = await db.query(
        "SELECT COUNT(*) AS cnt FROM created_events WHERE deleted_at IS NULL",
      );
      const total = Number(countRow?.cnt || 0);
      const offset = (p - 1) * pp;
      const pagedSql =
        baseSql + " ORDER BY ce.created_at DESC LIMIT ? OFFSET ?";
...
const getAttendanceRecordsByOsws = async (adminId) => {
  ...
  try {
    ...
    console.error(
      "Error fetching attendance records by OSWS admin:",
      error.stack,
    );
    throw error;
  }
...
const getAllOrgEvents = async (page = undefined, per_page = undefined) => {
  ...
  try {
    if (page && per_page) {
      ...
      const pp = Math.max(1, Math.min(200, parseInt(per_page, 10)));
      const [[countRow]] = await db.query(
        "SELECT COUNT(*) AS cnt FROM created_events WHERE created_by_org_id IS NOT NULL AND deleted_at IS NULL",
      );
      const total = Number(countRow?.cnt || 0);
      const offset = (p - 1) * pp;
      const pagedSql =
...
const getAllOswsEvents = async (page = undefined, per_page = undefined) => {
  ...
  try {
    if (page && per_page) {
      ...
      const pp = Math.max(1, Math.min(200, parseInt(per_page, 10)));
      const [[countRow]] = await db.query(
        "SELECT COUNT(*) AS cnt FROM created_events WHERE created_by_osws_id IS NOT NULL AND deleted_at IS NULL",
      );
      const total = Number(countRow?.cnt || 0);
      const offset = (p - 1) * pp;
      const pagedSql =
...
const updateEvent = async (eventId, eventData) => {
  ...
  if (event_latitude !== undefined || event_longitude !== undefined) {
    ...
    params.push(
      event_latitude !== undefined && event_latitude !== ""
        ? Number(event_latitude)
        : null,
    );
    params.push(
      event_longitude !== undefined && event_longitude !== ""
        ? Number(event_longitude)
        : null,
    );
  }
...
const getEventById = async (eventId) => {
  const [rows] = await db.query(
    "SELECT * FROM created_events WHERE event_id = ?",
    [eventId],
  );
  return rows[0];
};
...
const hardDeleteEvent = async (eventId) => {
  ...
  try {
    ...
    await conn.query("DELETE FROM attendance_records WHERE event_id = ?", [
      eventId,
    ]);
    ...
    await conn.query("DELETE FROM certificate_requests WHERE event_id = ?", [
      eventId,
    ]);
    ...
    await conn.query("DELETE FROM event_registrations WHERE event_id = ?", [
      eventId,
    ]);
...
module.exports = {
  ...
  autoUpdateEventStatuses: async () => {
    try {
      ...
      // Timezone handling: Convert stored local timestamp to UTC for consistent comparison against UTC_TIMESTAMP()
  ...
  ensureIsPaidColumn: async () => {
    try {
      ...
      if (!Array.isArray(rows) || rows.length === 0) {
        await db.query(
          `ALTER TABLE created_events ADD COLUMN is_paid TINYINT(1) NOT NULL DEFAULT 0 AFTER event_poster`,
        );
        try {
          await db.query(
            `UPDATE created_events SET is_paid = 0 WHERE is_paid IS NULL`,
          );
        } catch (_) {
          /* no-op */
        }
  ...
  ensureRegistrationFeeColumn: async () => {
    try {
      ...
      if (!Array.isArray(rows) || rows.length === 0) {
        await db.query(
          `ALTER TABLE created_events ADD COLUMN registration_fee DECIMAL(10,2) NOT NULL DEFAULT 0 AFTER is_paid`,
        );
        try {
          await db.query(
            `UPDATE created_events SET registration_fee = 0 WHERE registration_fee IS NULL`,
          );
        } catch (_) {}
      }
    } catch (e) {
      console.warn(
        "[DB] ensureRegistrationFeeColumn check failed:",
        e.message || e,
      );
    }
  },
  ...
  ensureRegistrationStatusColumns: async () => {
    try {
      ...
      if (!Array.isArray(scol) || scol.length === 0) {
        await db.query(
          `ALTER TABLE event_registrations ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'approved' AFTER qr_code`,
        );
      }
      // approved_at
      ...
      if (!Array.isArray(acol) || acol.length === 0) {
        await db.query(
          `ALTER TABLE event_registrations ADD COLUMN approved_at DATETIME NULL AFTER status`,
        );
      }
      // approved_by_org_id
      ...
      if (!Array.isArray(aborg) || aborg.length === 0) {
        await db.query(
          `ALTER TABLE event_registrations ADD COLUMN approved_by_org_id INT NULL AFTER approved_at`,
        );
      }
      // approved_by_osws_id
      ...
      if (!Array.isArray(abosws) || abosws.length === 0) {
        await db.query(
          `ALTER TABLE event_registrations ADD COLUMN approved_by_osws_id INT NULL AFTER approved_by_org_id`,
        );
      }
    } catch (e) {
      console.warn(
        "[DB] ensureRegistrationStatusColumns check failed:",
        e.message || e,
      );
    }
  },
  ...
  // Aggregated chart datasets for OSWS dashboard using UTC comparisons for accuracy
  getOswsDashboardCharts: async (filter) => {
    ...
    return {
      events_by_department: deptRows.map((r) => ({
        department: r.department || "Unknown",
        count: r.cnt || 0,
      })),
      activities_by_organization: orgRows.map((r) => ({
  ...
  getOswsDashboardStats: async () => {
    // Use UTC_TIMESTAMP() and event timezone conversion to compare timestamps reliably
    const nowQuery = "UTC_TIMESTAMP()";
    const eventTzOffset = process.env.EVENT_TZ_OFFSET || "+08:00";
  ...
  ensureAttendanceColumns: async () => {
    try {
      ...
      if (!Array.isArray(ti) || ti.length === 0) {
        await db.query(
          `ALTER TABLE attendance_records ADD COLUMN time_in DATETIME NULL AFTER student_id`,
        );
      }
      ...
      if (!Array.isArray(to) || to.length === 0) {
        await db.query(
          `ALTER TABLE attendance_records ADD COLUMN time_out DATETIME NULL AFTER time_in`,
        );
      }
      ...
      if (!Array.isArray(sb) || sb.length === 0) {
        await db.query(
          `ALTER TABLE attendance_records ADD COLUMN scanned_by_osws_id INT(11) NULL AFTER scanned_by_org_id`,
        );
      }
      ...
      if (!Array.isArray(sbs) || sbs.length === 0) {
        await db.query(
          `ALTER TABLE attendance_records ADD COLUMN scanned_by_student_id VARCHAR(20) NULL AFTER scanned_by_osws_id`,
        );
      }
      // Backfill time_in from attended_at
      try {
        await db.query(
          `UPDATE attendance_records SET time_in = attended_at WHERE time_in IS NULL AND attended_at IS NOT NULL`,
        );
      } catch (_) {}
    } catch (e) {
      console.warn(
        "[DB] ensureAttendanceColumns check failed:",
        e.message || e,
      );
    }
  },
