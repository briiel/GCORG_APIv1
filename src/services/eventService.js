...
const getAttendanceRecordsByStudent = async (studentId) => {
  try {
    ...
    console.error(
      "Error fetching attendance records by student in service:",
      error,
    );
    throw error;
  }
...
const getAttendanceRecordsByOrg = async (orgId) => {
  try {
    ...
    console.error(
      "Error fetching attendance records by org in service:",
      error,
    );
    throw error;
  }
...
const getAttendanceRecordsByOsws = async (adminId) => {
  try {
    ...
    console.error(
      "Error fetching attendance records by OSWS in service:",
      error,
    );
    throw error;
  }
...
// Get aggregate organization stats (upcoming/ongoing/cancelled omit trashed; concluded includes trashed)
const getOrgDashboardStats = async (orgId) => {
  return await eventModel.getOrgDashboardStats(orgId);
};

// Get aggregate OSWS dashboard stats (upcoming/ongoing/cancelled omit trashed; concluded includes trashed)
...
module.exports = {
  ...
  // Automated event start/finish/trash/email functionality temporarily or permanently disabled
  permanentDeleteEvent: async ({ eventId, user }) => {
    ...
    if (!ev.deleted_at)
      return { deleted: false, code: 400, message: "Event is not in trash" };
