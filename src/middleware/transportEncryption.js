// Seamless AES-256-GCM payload encryption middleware based on X-Encrypted header status
...
function getPayloadKey() {
  ...
  if (!hex || hex.length !== 64) {
    throw new Error(
      "[TransportEncryption] PAYLOAD_ENCRYPTION_KEY must be 64 hex chars (32 bytes).",
    );
  }
...
function decryptPayload(encryptedStr) {
  ...
  if (parts.length !== 3)
    throw new Error("[TransportEncryption] Invalid encrypted payload format.");
...
const decryptRequestBody = (req, res, next) => {
  ...
  try {
    ...
    const encryptedStr =
      typeof body === "string"
        ? body
        : body && typeof body.payload === "string"
          ? body.payload
          : null;
    if (!encryptedStr) {
      return res
        ...
        .json({
          success: false,
          message: "Encrypted request body missing or invalid.",
        });
    }
    ...
    console.error(
      "[TransportEncryption] Request decryption failed:",
      err.message,
    );
    return res
      .status(400)
...
const encryptResponseBody = (req, res, next) => {
  ...
  res.json = function (data) {
    try {
      ...
      console.error(
        "[TransportEncryption] Response encryption failed:",
        err.message,
      );
