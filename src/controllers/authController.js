...
const register = asyncHandler(async (req, res) => {
  ...
  if (!student_id || !email || !password || !first_name || !last_name) {
    throw new ValidationError(
      "Student ID, email, password, first name, and last name are required.",
    );
  }
  ...
  if (!emailRegex.test(email)) {
    throw new ValidationError(
      "Email must be a valid Gordon College email (format: 202211223@gordoncollege.edu.ph).",
    );
  }
  ...
  for (const student of allStudents) {
    if (safeDecrypt(student.email) === email) {
      emailExists = true;
      break;
    }
  }
  ...
  const encryptedMiddleInitial = middle_initial
    ? encryptData(middle_initial)
    : null;
